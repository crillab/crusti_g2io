...
impl<R> NamedParam<BoxedGenerator<R>> for TreeGeneratorFactory {
...
impl<R> GeneratorFactory<R> for TreeGeneratorFactory where R: Rng {}
...
mod tests {
    ...
    fn test_not_enough_params() {
        assert!(
            (TreeGeneratorFactory.try_with_params("") as Result<BoxedGenerator<ThreadRng>>)
                .is_err()
        )
    }
    ...
    fn test_too_much_params() {
        assert!(
            (TreeGeneratorFactory.try_with_params("1,1") as Result<BoxedGenerator<ThreadRng>>)
                .is_err()
        )
    }
    ...
    fn test_tree_of_zero() {
        ...
        let g = TreeGeneratorFactory.try_with_params("0").unwrap()(&mut rng);
    ...
    fn test_tree_of_one() {
        let mut rng = rand::thread_rng();
        let g = TreeGeneratorFactory.try_with_params("1").unwrap()(&mut rng);
    ...
    fn test_tree() {
        let mut rng = rand::thread_rng();
        let g = TreeGeneratorFactory.try_with_params("4").unwrap()(&mut rng);
